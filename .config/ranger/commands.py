...
class fd_prev(Command):
    ...
    def execute(self):
        if len(fd_deq) > 1:
            ...
            self.fm.select_file(fd_deq[0])
# For convenience you can map these in your ~/.config/ranger/rc.conf, get one with ranger --copy-config=rc. For example:
...
#         output = subprocess.check_output(["fasd", "-dl"], universal_newlines=True)
